...
<div>
    ...
    <div class='container-fluid'>
        <div class='row'>
            ...
            <div v-if='item && !empty' class='col-sm-12 mt-5 bg-white min-100vh text-dark pt-5 pb-5 col-md-8 col-lg-4'>
            ...
            <div v-if='empty' class='col-sm-12 bg-white d-flex align-items-center justify-content-center min-100vh text-dark col-md-8 col-lg-4'>
                Tidak ada resep - "{{ this.$route.params.key }}"
            </div>
            <div class='col-sm-12 col-md-2 col-lg-4'></div>
        </div>
    </div>
...
export default {
    ...
    mounted() {
        this.$axios('https://limitless-sea-61725.herokuapp.com/https://masak-apa-tomorisakura.vercel.app/api/search/?q=' + this.$route.params.key)
            .then(res => {
                this.item = [];
                let arr = res.data.results;
                let key = this.$route.params.key;
                let resSplit = key.split(' ');

                for (let r in resSplit) {
                    let ubahKeyOne = resSplit[r].slice(0, 1).toUpperCase();
                    let ubahKeyTwo = resSplit[r].slice(1, resSplit[r].length).toLowerCase();
                    let finalKey = ubahKeyOne + "" + ubahKeyTwo;

                    if (resSplit[r].toLowerCase() !== 'resep') {
                        let h = arr.filter(x => x.title.search(finalKey) !== -1);
            ...
            .catch(err => {
                alert('Ups... terjadi error! Silahkan refresh halaman ' + err)
            })
    }
}
</script>
